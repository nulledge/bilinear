...
config.GT.BI.param = '/media/nulledge/2nd/ubuntu/bilinear/save/Bilinear GT c2f-converted/parameter/160.save'
...
with tqdm(total=delta) as progress:
    for idx in range(31684+delta, 31684+delta*2):
        ...
        pose_2d = torch.Tensor(np.expand_dims(info[data.Annotation.Part], 0)).to(device)
